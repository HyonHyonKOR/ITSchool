...
public class PolyTest {
	...
	public static void main(String[] args) {
		...
		ms1.disp(); // 동적 바인딩: 26번 오버라이딩된 disp() 메서드 호출
		MiddleSchool ms2 = new MiddleSchool("행복중학교", "서울시");
		ms2.disp(); // 동적 바인딩: 26번 오버라이딩된 disp() 메서드 호출
		
		HighSchool hs1 = new HighSchool("사랑고등학교", "경기도");
		hs1.disp(); // 동적 바인딩: 11번 오버라이딩된 disp() 메서드 호출
		...
		sch1.disp();//동적 바인딩: 26번 오버라이딩된 disp() 메서드 호출
		
		//2. 다운캐스팅(강제형변환): 업캐스팅된 객체를 다시 원상대로 되돌리는 형변환
		...
		//자식 터입 <= 부모 타입(Down-casting) : 명시적 형변환(형변환 생략불가)
